...
from config import add_train_vae_args,add_img_parser
from img_data import PartNetDataset, Tree
import utils
...
def train(conf):
    ...
    image_encoder = models.ImagePCEncoder(mask_load_path="/home/server/workspace/fuckmah/code/checkpoint/model_149.pkl", code_length=512)
    decoder = models.RecursiveDecoder(conf)
    models = [image_encoder,encoder,  decoder]
    model_names = ['image_encoder','encoder', 'decoder']
    ...
    # create optimizers
    encoder_opt = torch.optim.Adam(image_encoder.parameters(), lr=conf.lr)
    ...
    if not conf.no_console_log:
        header = '     Time    Epoch     Dataset    Iteration    Progress(%)      LR       LatentLoss    GeoLoss   CenterLoss   ScaleLoss   StructLoss  EdgeExists  KLDivLoss   SymLoss   AdjLoss   DiffLoss   TotalLoss'
    ...
    for epoch in range(conf.epochs):
        ...
        for train_batch_ind, batch in train_batches:
            ...
            total_loss = forward(
                batch=batch, data_features=data_features, encoder=image_encoder,part_encoder=encoder, decoder=decoder, device=device, conf=conf,
...
def forward(batch, data_features, encoder, part_encoder,decoder, device, conf,
    ...
    images = batch[data_features.index('image')]
    ...
    losses = {
        ...
        'diffs': torch.zeros(1, device=device)
    }
    ...
    for obj, img in zip(objects, images):
        ...
        box_code = part_encoder.encode_structure(obj=obj)

        # get kldiv loss
        if not conf.non_variational:
            box_code, box_kldiv_loss = torch.chunk(box_code, 2, 1)
            img_code, img_kldiv_loss = torch.chunk(img_code, 2, 1)
            img_kldiv_loss = -img_kldiv_loss.sum()
            box_kldiv_loss = -box_kldiv_loss.sum()
            losses['kldiv'] = losses['kldiv'] + box_kldiv_loss
            losses['diffs'] = losses['diffs'] + torch.sqrt(torch.sum((box_code - img_code) ** 2))

        # decode root code to get reconstruction loss
        obj_losses = decoder.structure_recon_loss(z=box_code, gt_tree=obj)
    ...
    losses['diffs'] *= conf.loss_weight_diff
    ...
    with torch.no_grad():
        ...
        if log_tb and tb_writer is not None:
            ...
            tb_writer.add_scalar('diff_loss', losses['diffs'].item(), step)

...
from img_data import Tree
from utils import linear_assignment, load_pts
from model_img import fullmodel,ImageEncoder

class ImagePCEncoder(nn.Module):
    def __init__(self, mask_load_path, code_length):
        super(ImagePCEncoder, self).__init__()
        mask_estimator = fullmodel()
        # print('mask_load_path is ', mask_load_path)
        mask_estimator = torch.load(mask_load_path)
        self.img_encoder = ImageEncoder(mask_estimator=mask_estimator,code_length=code_length)
    def forward(self, x):
        output = self.img_encoder(x)
        return output
